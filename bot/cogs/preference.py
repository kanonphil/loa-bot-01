...
class Preference(commands.Cog):
    ...
    @app_commands.describe(시간="알림을 받을 시간 (예: 1 = 1시간 전, 0.5 = 30분 전)")
    @app_commands.choices(시간=[
    ...
    async def set_pre_notify(self, interaction: discord.Interaction, 시간: float) -> None:
        discord_id = str(interaction.user.id)
        await db.set_pre_notify_hours(discord_id, 시간)
