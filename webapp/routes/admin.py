...
def _redirect(fallback_reason: str, result: dict, raid: str | None = None) -> RedirectResponse:
    base = "/admin/raids" + (f"?raid={quote(raid)}" if raid else "")
    if not result.get("success"):
        reason = quote(result.get("reason") or fallback_reason)
        sep = "&" if raid else "?"
        return RedirectResponse(f"{base}{sep}error={reason}", status_code=303)
    return RedirectResponse(base, status_code=303)


@router.get("/admin/raids")
async def admin_raids_page(
    request: Request, error: str | None = None, raid: str | None = None,
...
async def add_category(
    ...
    return _redirect("카테고리를 추가하지 못했습니다. (이미 있는 이름일 수 있어요)", result)
...
async def delete_category(name: str = Form(...), user: dict = Depends(require_admin)):
    ...
    return _redirect("카테고리를 삭제하지 못했습니다.", result)
...
async def set_category_extreme(
    ...
    return _redirect("변경하지 못했습니다.", result)
...
async def add_raid(
    ...
    return _redirect("레이드를 추가하지 못했습니다. (이미 있는 이름일 수 있어요)", result)
...
async def delete_raid(name: str = Form(...), user: dict = Depends(require_admin)):
    ...
    return _redirect("레이드를 삭제하지 못했습니다.", result)
...
async def add_difficulty(
    ...
    return _redirect("난이도를 추가하지 못했습니다. (이미 있는 이름일 수 있어요)", result, raid=raid_name)
...
async def delete_difficulty(
    ...
    return _redirect("난이도를 삭제하지 못했습니다.", result, raid=raid_name)
...
async def add_class(
    ...
    return _redirect("직업을 추가하지 못했습니다. (이미 있는 이름일 수 있어요)", result)
...
async def delete_class(name: str = Form(...), user: dict = Depends(require_admin)):
    ...
    return _redirect("직업을 삭제하지 못했습니다.", result)
